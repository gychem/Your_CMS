...
<div class="w-full max-w-xxl">
    <form action="/admin/news/authors/{{ $data['author']->username }}/search" class="w-full flex gap-2 text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-white p-2 rounded" method="POST">
    @csrf
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 mt-2">
            ...
            <tbody>
                @foreach ($data['articles'] as $article)   
                    <tr onclick="window.location='/news/{{ $article->slug }}';" class="cursor-pointer bg-white hover:bg-gray-50 border-b dark:bg-gray-700 dark:border-gray-600 dark:hover:bg-slate-600">
                        <td class="py-4 px-6">
                            {{ $article->id }}
                        </td>
                        <td class="py-4 px-6">
